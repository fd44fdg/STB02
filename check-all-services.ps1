...
Write-Host "🔍 检查掌上刷题宝所有服务状态..." -ForegroundColor Green
Write-Host ""
...
Write-Host "   - H5前端用户界面: http://localhost:8083 (学生用户界面) ✅" -ForegroundColor Green
Write-Host "   - H5前端用户界面: http://localhost:8084 (备用界面) ✅" -ForegroundColor Green
...
Write-Host "   📱 学生用户: 访问 http://localhost:8083 或 http://localhost:8084" -ForegroundColor White
...
Write-Host ""
Write-Host "💡 提示：后端API未启动，前端功能可能受限。如需完整功能，请配置数据库后启动后端服务。" -ForegroundColor Magenta
